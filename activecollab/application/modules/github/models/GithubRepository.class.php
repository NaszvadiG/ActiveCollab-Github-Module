	...
	class GithubRepository extends ProjectObject {
		...
		function testRepositoryConnection()
		{
			if( ($url_info = $this->userRepoFromUrl()) !== False) {
				...
				if(is_object($response = $this->githubAPIRequest($request_url))) {
					return True;
				} else {
					return $response;
				}
		...
		{
			$http_params = Array('method' => 'GET');
			
			if( ($api_key = $this->getFieldValue('varchar_field_1')) != '' && 
						($username = $this->getFieldValue('varchar_field_2')) != '') {
				$username .= '/token';
				$http_params['header'] = Array(sprintf("Authorization: Basic %s\r\n", base64_encode($username.':'.$api_key)));
			}
			$context = stream_context_create(array('http' => $http_params));
